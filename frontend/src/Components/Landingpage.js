...
function Landingpage(props) {
  ...
  async function handlesort(sortType){    
    ...
    if(sortType==="Release date"){
      let sortedvidoesbydate =filtvideos.sort( (v1,v2)=>(new Date(v1.releaseDate).getTime()<new Date(v2.releaseDate).getTime())?1:(new Date(v1.releaseDate).getTime()>new Date(v2.releaseDate).getTime())? -1 : 0)
      console.log(sortedvidoesbydate)
      setfilteredvideos(sortedvidoesbydate)
      return;
    }else if(sortType==="View_Count"){
      let sortedvidoesbyview =filtvideos.sort( (v1,v2)=>(v1.viewCount<v2.viewCount)?1:(v1.viewCount>v2.viewCount)? -1 : 0)
  ...
  return (
    <Box>
      ...
      {isLoading?(<Box
        ...
        >
          <CircularProgress />
        </Box>):(<Dashboard videos={filteredvideos} />)}
